...
* **Радиус кнопок — поле `radius` в `em`**, рендерер выводит его инлайн-стилем, и он перекрывает
  вендорский CSS (`2px`). Считай от шрифта кнопки: `radius = 7 / font_size_px` (0.389 при 18px).
* **Цвет текста кнопок в формах полем не задаётся** — работает `var(--autocontrast-text-color)`:
  светлый фон → тёмный текст, тёмный → белый. Меняешь фон — перепроверяй вычисленный `color`.
* **Меняй модель блока и перерисовывай**, а не подпирай CSS: блок «Пользовательский код» с
  `!important` расходится с настройками редактора и забывается.
