...
Прежде чем выдумывать обходной путь, посмотри в `skills/tobiz-mcp/references/`:

* `block-catalog.md` — 28 категорий и 162 типа блоков: `type_id`, название, назначение;
* `block-engine.md` — `values`/`cache`, каркас секции, общие поля (фон, отступы, ширина, срез,
  плашка под текстом), картинки, массивы, все рендереры вендора;
* `components.md` — кнопки (радиус в em, автоконтраст текста), формы (17 типов полей, маски,
  капча, попапы), меню (уровни через `level`, бургер, dropdown), карты, лого/телефоны/соцсети,
  видео, таймеры, quiz, калькулятор;
* `panel-api.md` — внутренние эндпоинты, формат ответов, коды ошибок, границы возможностей.
