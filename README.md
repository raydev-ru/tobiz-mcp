...
В скиле: что пишется сразу, а что только по `tobiz_save_page`; 14 проверенных грабель (радиус
кнопок в `em`, автоконтраст текста в формах, обязательный `block_id` при загрузке картинок,
отстающий `window.tobiz`, `?v=` = id страницы) и короткие рецепты под типовые задачи.
